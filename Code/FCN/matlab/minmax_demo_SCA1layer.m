...
imgRoot = 'E:\Datasets\Database-S\ECSSD\ECSSD-Image\';% test image path
matRoot = ['E:\Datasets\Database-S\ECSSD\ECSSD_VGG-NET_FeatureMap-mat2\'];
edgeRoot = ['E:\Datasets\Database-S\ECSSD\ECSSD_Edges\'];

% saldir1 = './vgg37pm_ECSSD/';
% saldir = ['.\vggfeat63612_ECSSD\'];
saldir = ['.\maxpool_ECSSD\'];
% mkdir(saldir);
...
%         imwrite(IM_B,[saldir,imnames(ii).name(1:end-4),'_suRGB.png']);
        [sulabel,impfactor8,pixelList] = SLIC_Split(input_im, spnumber);
%         IM_B = myDrawBounderis(sulabel,input_im);
%         imwrite(IM_B,[saldir,imnames(ii).name(1:end-4),'_suLab.png']);
%         continue;
        supNum = max(sulabel(:));
...
%     bdIds = extract_bg_sp(sulabel,m,n);
    bdIds = GetBndPatchIds(sulabel,8);
%     S_N1 = zeros(supNum,1);
%     S_N1(bdIds)=1;
...
%         disp(process);
%         saldir = ['.\vgg',num2str(layer),'pm_ECSSD\'];
%         if ~exist(saldir, 'dir')
%             mkdir(saldir);
%         end
