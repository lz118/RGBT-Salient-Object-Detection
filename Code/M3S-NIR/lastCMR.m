function [s,S]=lastCMR(Y,index,H1,H2,I,L1,L2,c,s)
...
for v=1:3
    ...
    Y1{v}=Y{v}(1:index{v});
    Y2{v}=Y{v}(index{v}+1:2*index{v});
    
    temp_f1{v}= zeros(index{v},1);
    temp_f2{v}= zeros(index{v},1);
    %求f的结果，单独两个模态做
    q1{v}=zeros(index{v},1);
    q2{v}=zeros(index{v},1);
    temp_Q{v}=Y{v};%初始的Q等于初始的种子节点
   ...
   while iter<=maxIter  %%迭代优化  
    ...
    for v=1:3
        ...
        e1=f1{v}-y1{v}/mu;
        e2=f2{v}-y2{v}/mu;
        q1{v}= softthreshold(S1{v},e1,Y1{v},lambda,mu/2,lambda2,index{v});
        q2{v}= softthreshold(S2{v},e2,Y2{v},lambda,mu/2,lambda2,index{v});
        Q{v}=[q1{v};q2{v}];
